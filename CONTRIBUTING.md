...
* **Migration Flag Upgrade Rule**: If a released version has already written a migration flag (for example `v1`) into player saves, and you later need to correct that migration, do not keep using the same flag. Instead, clear the old flag/state through a `State` cleanup function, switch that migration to a brand-new flag (for example `v2`), and make new saves write the new flag directly in `State.setup_new_game()`. In other words: the migration function itself may be reused, but the persisted flag it relies on must be upgraded.
...
* **迁移标记升级原则**：如果某个已发布版本已经把一个迁移标记（例如 `v1`）写进了玩家存档，而你后来又需要修正这条迁移，就不要继续沿用旧标记。正确做法是：通过 `State` 中的清理函数清掉旧标记和旧状态，把这条迁移切换到一个全新的标记（例如 `v2`），并让新存档在 `State.setup_new_game()` 中直接写入新标记。也就是说，迁移函数本体可以复用，但它依赖的持久化标记必须升级。
