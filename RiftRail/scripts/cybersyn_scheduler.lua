...
local function insert_cybersyn_stop_sequence(new_records, original_records, target_station_data, station_type_name, train_surface_index)
...
local function process_train(train)
    ...
    if s_D ~= s_P then
        ...
        if portal_name then
            table.insert(new_records, { station = portal_name, temporary = true, wait_conditions = { { type = "time", ticks = 0 } } })
        end
    end
